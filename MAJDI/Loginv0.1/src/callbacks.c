...
if (verifier(login,password) !=-1) {
  suprimer(login,password);
  gtk_label_set_text(GTK_LABEL(output),"La supression a été éffectué avec succéss");
}
else
  gtk_label_set_text(GTK_LABEL(output),"La combination Login/Password n'existe pas");
}
